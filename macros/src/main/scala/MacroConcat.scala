...
/**
  *
  * Investigation of fast string interpolation described at
...
object MacroConcat {
  ...
  def soImpl(c: whitebox.Context)(args: c.Expr[Any]*): c.Expr[String] = {
    ...
    if (args.isEmpty) c.Expr(Literal(Constant(constantParts.mkString(""))))
      ...
      val allParts = getAllPartsForAppend(c)(constantParts, arguments).toList

      // code generation

      val plusLenExpr = lengthSum(c)(initialLength, lenNames.flatten.toList)

      val newStringBuilderAndAppendExpr = newStringBuilderWithAppends(c)(
        q"new java.lang.StringBuilder($plusLenExpr)",
        allParts.reverse)
  ...
  def sfiImpl(c: whitebox.Context)(args: c.Expr[Any]*): c.Expr[String] = {
    ...
    val (valDeclarations, lenNames, arguments: Seq[c.universe.Tree]) =
    ...
    val stringBuilderWithAppends = newStringBuilderWithAppends(c)(
      q"new java.lang.StringBuilder(len)",
      allParts.reverse)
  ...
  private def getAllPartsForAppend(c: whitebox.Context)(
      rawParts: Seq[String],
      arguments: Seq[c.universe.Tree]) = {
    import c.universe._
  ...
  private def lengthSum(c: whitebox.Context)(
      const: Int,
      names: List[c.universe.TermName]): c.universe.Tree = {
    import c.universe._

    if (names.isEmpty) {
      Literal(Constant(const))
    } else {
      Apply(Select(Select(Ident(names.head), TermName("length")),
                   TermName("$plus")),
  ...
  private def newStringBuilderWithAppends(c: whitebox.Context)(
    ...
    if (expressions.isEmpty) {
      ...
      Apply(Select(newStringBuilderWithAppends(c)(newStringBuilderExpr,
                                                  expressions.tail),
                   TermName("append")),
